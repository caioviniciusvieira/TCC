                    ...
                    <li class="user-dropdown-container">
                        ...
                        <ul class="user-dropdown-menu">
                            <li class="user-logout">
                                <a href='<?php echo $link_cadastro_html;?>'>
        ...
        <div class="account-wrapper">
            ...
            <section class="cart">
                ...
                <div class="cart-items">
                    <?php
                    $sql_carrinho = $conexao->prepare("SELECT * FROM lista_desejo WHERE nome_completo = ? ");
                    $sql_carrinho->bind_param('s', $_SESSION['nome']);
                    $sql_carrinho->execute();
                    $query_carrinho = $sql_carrinho->get_result();
                    $num_carrinho = $query_carrinho->num_rows;
                    ...
                    if($num_carrinho == 0){
                        ...
                        while($carrinho = $query_carrinho->fetch_assoc()){
                            echo "
                            <form method='post' action='../src/backend/deletar_carrinho.php'>  
                                <a href='' class='cart-item'>
                                    <img class='item' src='".$carrinho['imagem']."' alt='Produto' />
                                    <div class='cart-info'>
                                        <h3 class='title-item'>".$carrinho['nome_produto']."</h3>
                                        <p class='subtitle-item'>".$carrinho['descricao']."</p>
                                        <span class='price'>".$carrinho['valor']."</span>
                                        <div class='cart-controls'>
                                            <form method='post' action='../src/backend/deletar_carrinho.php'>
                                                <input type='hidden' name='nome_produto' value='" . $carrinho['nome_produto'] . "'>
                                                <button name='deletar' class='remove'>Remover</button> 
                                            </form>
                                            <form method='post' action='../src/backend/verificaCompra.php?id=$id'>                       
                                                <button name='comprar' class='comprar'>Comprar</button>
                                            </form>
                                        </div>
                                    </div>
                                </a>
